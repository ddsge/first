...
def predefined_dataset(name, binary_y=False):
    ...
    if name == 'adult':
        ...
        table = pd.read_csv(dir_path + '/fit/fitdata_new.csv', header=0, na_values='?', skipinitialspace=True).dropna()
        table_X = table.iloc[:, :-1].copy()
        table_Y = table.iloc[:, -1].copy()
        categorical_cols = None
        numerical_cols = None
...
def transform_dataset(name, method='ordinal', negations=False, labels='ordinal'):
    ...
    if method in ['onehot', 'onehot-compare']:
        ...
        # featureBinarizer ()괄호안의 기준으로 나눈다, colCateg= object type 데이터를 categorical 데이터로 설정
